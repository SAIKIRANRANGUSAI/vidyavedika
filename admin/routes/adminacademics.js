...
// GET form page (Academics & Calendar)
router.get("/", async (req, res) => {
    try {
        const [academicsRows] = await db.query("SELECT * FROM academics_section LIMIT 1");
        const academics = academicsRows.length ? academicsRows[0] : {};

        const [calendarRows] = await db.query("SELECT * FROM academic_calendar LIMIT 1");
        const calendar = calendarRows.length ? calendarRows[0] : {};

        const [sectionRows] = await db.query("SELECT * FROM exam_section LIMIT 1");
        const section = sectionRows.length ? sectionRows[0] : {};

        const [rows] = await db.query("SELECT * FROM admissions_section LIMIT 1");
        const admissions = rows.length ? rows[0] : {};

        // ✅ Just pass section, no sectionr, no items
...
// POST save/update Academics Section
router.post("/save", upload.single("image"), async (req, res) => {
    try {
        const { heading, sub_heading, list_content } = req.body;
        const imagePath = req.file ? "/uploads/academics/" + req.file.filename : null;

        const [rows] = await db.query("SELECT id FROM academics_section LIMIT 1");

        if (rows.length) {
            await db.query(
                `UPDATE academics_section SET heading=?, sub_heading=?, list_content=?, image=COALESCE(?, image) WHERE id=?`,
                [heading, sub_heading, list_content, imagePath, rows[0].id]
            );
        } else {
            await db.query(
                `INSERT INTO academics_section (heading, sub_heading, list_content, image) VALUES (?, ?, ?, ?)`,
                [heading, sub_heading, list_content, imagePath]
            );
        }

        // Fetch updated rows
        const [updatedAcademics] = await db.query("SELECT * FROM academics_section LIMIT 1");
        const [calendarRows] = await db.query("SELECT * FROM academic_calendar LIMIT 1");

        res.render("academic_s", { academics: updatedAcademics[0] || {}, calendar: calendarRows[0] || {} });
    } catch (err) {
    ...
    try {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            items[`item${i}_image`] = req.files[`item${i}_image`]
                ? "/uploads/exams/" + req.files[`item${i}_image`][0].filename
                : null;
        }
...
router.post("/admissions/save", upload.fields([
    ...
    try {
        ...
        ["image1", "image2", "image3"].forEach((img, idx) => {
            images[img] = req.files[img] ? "/uploads/academics/" + req.files[img][0].filename : null;
        });
